// Generated by purs version 0.15.16
import * as Data_Array from "../Data.Array/index.js";
import * as Data_Functor from "../Data.Functor/index.js";
import * as Data_Maybe from "../Data.Maybe/index.js";
import * as Data_Show from "../Data.Show/index.js";
import * as Data_String_Common from "../Data.String.Common/index.js";
import * as Puruby_RubyAst from "../Puruby.RubyAst/index.js";
var map = /* #__PURE__ */ Data_Functor.map(Data_Functor.functorArray);
var show = /* #__PURE__ */ Data_Show.show(Data_Show.showInt);
var printExpr = function (v) {
    if (v instanceof Puruby_RubyAst.RubyString) {
        return "\"" + (v.value0 + "\"");
    };
    if (v instanceof Puruby_RubyAst.RubyCall) {
        var fnStr = printExpr(v.value0);
        var argsStr = Data_String_Common.joinWith(", ")(map(printExpr)(v.value1));
        var $9 = fnStr === "TODO_Var";
        if ($9) {
            return "puts " + argsStr;
        };
        return fnStr + (".call(" + (argsStr + ")"));
    };
    if (v instanceof Puruby_RubyAst.RubyFunction) {
        return "-> {\x0a  " + (printExpr(v.value0) + "\x0a}");
    };
    if (v instanceof Puruby_RubyAst.RubyAccessor) {
        return printExpr(v.value0) + ("[\"" + (v.value1 + "\"]"));
    };
    if (v instanceof Puruby_RubyAst.RubyIndexAccess) {
        return printExpr(v.value0) + ("[" + (show(v.value1) + "]"));
    };
    if (v instanceof Puruby_RubyAst.RubyTernary) {
        return "(" + (printExpr(v.value0) + (" ? " + (printExpr(v.value1) + (" : " + (printExpr(v.value2) + ")")))));
    };
    if (v instanceof Puruby_RubyAst.RubyAbs) {
        var $20 = Data_Array.length(v.value0) === 0;
        if ($20) {
            return "lambda { " + (printExpr(v.value1) + " }.call");
        };
        return Data_Array.foldr(function (arg) {
            return function (acc) {
                return "lambda { |" + (arg + ("| " + (acc + " }")));
            };
        })(printExpr(v.value1))(v.value0);
    };
    if (v instanceof Puruby_RubyAst.RubyUncurriedAbs) {
        var $23 = Data_Array.length(v.value0) === 0;
        if ($23) {
            return "lambda { " + (printExpr(v.value1) + " }.call");
        };
        return "lambda { |" + (Data_String_Common.joinWith(", ")(v.value0) + ("| " + (printExpr(v.value1) + " }")));
    };
    if (v instanceof Puruby_RubyAst.RubyWhileTrue) {
        return "lambda { " + (Data_String_Common.joinWith("")(map(function (arg) {
            return "__tco_" + (arg + (" = " + (arg + "; ")));
        })(v.value0)) + ("loop do; " + ("__tco_res = catch(:tco_loop) do; " + (printExpr(v.value1) + ("; " + ("end; " + ("if __tco_res.is_a?(Array) && __tco_res[0] == :__tco_continue; " + (Data_String_Common.joinWith("")(Data_Array.mapWithIndex(function (i) {
            return function (arg) {
                return "__tco_" + (arg + (" = __tco_res[1][" + (show(i) + "]; ")));
            };
        })(v.value0)) + ("next; " + ("else; " + ("break __tco_res; " + ("end; " + ("end " + "}.call")))))))))))));
    };
    if (v instanceof Puruby_RubyAst.RubyContinue) {
        return "throw(:tco_loop, [:__tco_continue, [" + (Data_String_Common.joinWith(", ")(map(printExpr)(v.value1)) + "]])");
    };
    if (v instanceof Puruby_RubyAst.RubyHash) {
        return "{\x0a  " + (Data_String_Common.joinWith(",\x0a  ")(map(function (v1) {
            return "\"" + (v1.value0 + ("\" => " + printExpr(v1.value1)));
        })(v.value0)) + "\x0a}");
    };
    if (v instanceof Puruby_RubyAst.RubyArray) {
        return "[" + (Data_String_Common.joinWith(", ")(map(printExpr)(v.value0)) + "]");
    };
    if (v instanceof Puruby_RubyAst.RubyBinOp) {
        return "(" + (printExpr(v.value1) + (" " + (v.value0 + (" " + (printExpr(v.value2) + ")")))));
    };
    if (v instanceof Puruby_RubyAst.RubyGlobalVar) {
        var sanitize = function (n) {
            return Data_String_Common.replaceAll("$")("")(Data_String_Common.replaceAll("'")("_")(n));
        };
        if (v.value0 instanceof Data_Maybe.Just) {
            return "$" + (v.value0.value0 + ("_" + sanitize(v.value1)));
        };
        if (v.value0 instanceof Data_Maybe.Nothing) {
            return sanitize(v.value1);
        };
        throw new Error("Failed pattern match at Puruby.Printer (line 68, column 8 - line 70, column 31): " + [ v.value0.constructor.name ]);
    };
    if (v instanceof Puruby_RubyAst.RubyLocal) {
        return v.value0;
    };
    if (v instanceof Puruby_RubyAst.RubyLet) {
        return "(" + (v.value0 + (" = " + (printExpr(v.value1) + ("; " + (printExpr(v.value2) + ")")))));
    };
    if (v instanceof Puruby_RubyAst.RubyBlock) {
        return "begin\x0a" + (Data_String_Common.joinWith("\x0a")(map(printExpr)(v.value0)) + "\x0aend");
    };
    if (v instanceof Puruby_RubyAst.RubyRaw) {
        return v.value0;
    };
    if (v instanceof Puruby_RubyAst.RubyLetRec) {
        return "\"TODO: LetRec\"";
    };
    if (v instanceof Puruby_RubyAst.RubyAssign) {
        return "$" + (v.value0 + (" = " + printExpr(v.value1)));
    };
    throw new Error("Failed pattern match at Puruby.Printer (line 11, column 13 - line 79, column 43): " + [ v.constructor.name ]);
};
var printFile = function (file) {
    return Data_String_Common.joinWith("\x0a\x0a")(map(printExpr)(file.decls)) + "\x0a";
};
export {
    printExpr,
    printFile
};
//# sourceMappingURL=index.js.map
